...
drop policy if exists "own stress_history" on stress_history;
drop policy if exists "own ema" on ema_checkins;
drop policy if exists "own telemetry" on telemetry_events;
drop policy if exists "own baselines" on user_baselines;
drop policy if exists "own interventions" on interventions;
drop policy if exists "own chat messages" on chat_messages;
drop policy if exists "own focus" on focus_snapshots;
...
create policy "own insights" on wellness_insights for all using (auth.uid()::text = user_id);
