...
def _load_swellkw_file(path: str, condition_map: Optional[dict]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
    ...
    # session_duration_min: CONFOUNDED with experimental design (T blocks are
    # shorter by protocol; file order N-first). A model must not use it.
    # Slot kept for contract compatibility, value forced to 0.
    minute_counter = None
    ...
    # hour_of_day / day_of_week: CONFOUNDED with the study schedule (N blocks
    # ran at hour ~12.5, I/T at ~14.0). Zeroed for SWELL — the model must not
    # use clock time as a condition proxy.
    h_idx = FEATURE_NAMES.index("hour_of_day")
    d_idx = FEATURE_NAMES.index("day_of_week")
    X[:, h_idx] = 0.0
    X[:, d_idx] = 0.0
    stress = None
...
def _build_matrix(df: pd.DataFrame, labels: List[int], subject_col: Optional[str],
    ...
    # session_duration_min: confounded with experimental design (T blocks are
    # shorter by protocol; file order N-first). Forced to 0; slot kept only
    # for feature-contract compatibility.
...
def safe_predict(model, X) -> np.ndarray:
    """xgboost 3.4 predict() returns 2-D (n, num_class) when a class is
    missing from training data. Normalize to 1-D class labels everywhere."""
        ...
        for held in unique:
            ...
            if chronological:
                ...
                for s in np.unique(subjects[va_mask]):
                    ...
                    y_pred_all.extend(safe_predict(model, X_va).tolist())
                    for k in all_fold_scores:
                        ...
                        all_fold_scores[k].append(fn(y_va, safe_predict(model, X_va), **kw))
                    fold_rows.append({
                        "held_out_subject": str(s),
                        "split": "chronological_within_subject",
                        "macro_f1": float(f1_score(y_va, safe_predict(model, X_va),
                                                   average="macro", zero_division=0.0)),
                    })
                continue

            model.fit(X_tr, y_tr)
            y_pred = safe_predict(model, X_va)
            y_true_all.extend(y_va.tolist())
